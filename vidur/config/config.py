...
class ReplicaConfig:
    ...
    # ============================================================
    # [EP] EP 。 EP  cluster world_size
    # (tp×pp×dp)， cluster.py 。 != world_size，
    #  ValueError。
    ...
    def __post_init__(self):
        ...
        if self.expert_model_parallel_size != 1 and self.expert_model_parallel_size != self.world_size:
            raise ValueError(
                f"EP 。 EP={self.expert_model_parallel_size}，"
                f" world_size={self.world_size}。EP  world_size。"
                f" --replica_config_expert_model_parallel_size  {self.world_size}。"
            )
        self.expert_model_parallel_size = self.world_size  # Auto-set, overridden again in cluster.py / , cluster.py 
        
        #  ReplicaConfig  | Print ReplicaConfig summary
...
class BaseExecutionTimePredictorConfig(BasePolyConfig):
    ...
    simai_simulation_config: str = field(
        # default="./SimAI/astra-sim-odyn/inputs/config/SimAI.conf",
        # default="../SimAI/astra-sim-odyn/inputs/config/SimAI.conf",
        default="../astra-sim-odyn/inputs/config/SimAI.conf",
...
class ClusterConfig:
    ...
    # global_scheduler_config：，(RoundRobin)
    ...
    # replica_scheduler_config：， Sarathi 
