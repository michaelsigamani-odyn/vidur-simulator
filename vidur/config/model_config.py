...
class Qwen3Next80BA3BModelConfig(BaseModelConfig):
    ...
    # Fields mapped from base class parameters / 
    num_layers: int = 48  # maps to num_hidden_layers /  num_hidden_layers
    num_q_heads: int = 16  # maps to num_attention_heads /  num_attention_heads
    num_kv_heads: int = 2  # maps to num_key_value_heads /  num_key_value_heads
    embedding_dim: int = 2048  # maps to hidden_size /  hidden_size
    mlp_hidden_dim: int = 5120  # maps to intermediate_size /  intermediate_size
    use_gated_mlp: bool = True  # per model arch / 
    use_bias: bool = False  # per model arch / 
    use_qkv_bias: bool = False  # per model arch / 
    activation: ActivationType = ActivationType.SILU  # maps to hidden_act /  hidden_act
    norm: NormType = NormType.RMS_NORM  # per model arch / 
    post_attn_norm: bool = True  # per model arch / 
...
class Qwen3Moe235BA22BModelConfig(BaseModelConfig):
    ...
    vocab_size: int = 151936
    # Fields mapped from base class parameters / 
    num_layers: int = 94  # maps to num_hidden_layers /  num_hidden_layers
    num_q_heads: int = 64  # maps to num_attention_heads /  num_attention_heads
    num_kv_heads: int = 4  # maps to num_key_value_heads /  num_key_value_heads
    embedding_dim: int = 4096  # maps to hidden_size /  hidden_size
    mlp_hidden_dim: int = 12288  # maps to intermediate_size /  intermediate_size
    use_gated_mlp: bool = True  # per model arch / 
    use_bias: bool = False  # per model arch / 
    use_qkv_bias: bool = False  # per model arch / 
    activation: ActivationType = ActivationType.SILU  # maps to hidden_act /  hidden_act
