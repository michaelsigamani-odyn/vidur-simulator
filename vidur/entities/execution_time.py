...
# execution_time.py is under odyn-simulator/vidur/entities/
# aicb is under workspace_root/aicb/
# Path: entities/ -> vidur/ -> odyn-simulator/ -> workspace_root/ -> aicb/
...
class AICBGlobalCache:
    ...
    def __init__(self):
        ...
        # Statistics counters / 
        self._stats = {
            'cache_hits': 0,        # Exact cache hits / 
            'interpolations': 0,    # Interpolation hits / 
            'aicb_calls': 0,        # Actual AICB subprocess calls / AICB subprocess
            'csv_loads': 0,         # CSV file load count / CSV
        }
        ...
        self._log(f"AICBGlobalCache , : {_AICB_CACHE_DIR}")
    ...
    def get(self, model_name, ws, tp, pp, ep, bs, seq, phase) -> Optional[Dict]:
        ...
        if key in self._cache:
            ...
            self._log(f"[] : key={self._format_key(key)}, "
    ...
    def put(self, model_name, ws, tp, pp, ep, bs, seq, phase, data: Dict) -> None:
        """Store AICB data into cache / AICB"""
        key = self._make_key(model_name, ws, tp, pp, ep, bs, seq, phase)
        self._cache[key] = data
        self._log(f"[] key={self._format_key(key)}, layers={len(data)}")
        
        # Persist to disk cache / 
        self._save_cache_entry(key, data)
    
    def record_aicb_call(self) -> None:
        """Record an actual AICB subprocess call / AICB subprocess"""
        self._stats['aicb_calls'] += 1
        self._log(f"[AICB] {self._stats['aicb_calls']}subprocess")
    
    def record_csv_load(self) -> None:
        """Record a CSV file load / CSV"""
    ...
    def _find_neighbors(self, group_key, target_seq) -> Optional[Dict]:
        ...
        - One neighbor: nearest neighbor / : 
        - No neighbors: return None / : None
        """
        ...
        if lower_seq is not None and upper_seq is not None and lower_seq != upper_seq:
            ...
            self._log(f"  : seq={target_seq}, lower={lower_seq}, upper={upper_seq}, "
                      f"alpha={alpha:.4f}")
            return interpolated
        ...
        if nearest is not None:
            self._log(f"  : seq={target_seq}, nearest={nearest}")
    ...
    def _interpolate(self, data_low: Dict, data_high: Dict, alpha: float) -> Dict:
        ...
        Args:
            data_low: AICB data at lower seq / seqAICB
            data_high: AICB data at higher seq / seqAICB
    ...
    def _format_key(self, key: Tuple) -> str:
        """Format key as a human-readable string / key"""
    ...
    def _load_index(self) -> None:
        """Load cache index and existing data from disk / """
        try:
    ...
    def print_stats(self) -> None:
        ...
        self._log(f": hits={self._stats['cache_hits']}, "
    ...
    def save_lookup_table(self) -> None:
        """Save complete lookup index to JSON for inspection / JSON，"""
        try:
            table = {}
            for key, data in self._cache.items():
...
class ExecutionTime(BaseEntity):
    ...
    def _get_moe_layer_execution_time_from_aicb(self, layer_id) -> float:
        ...
        if replica_stage == "prefill": # normal kernel
            # Convert Gbps to Byte/s / Gbps Byte/s
    ...
    def _get_aicb_params(self):
        ...
        These values are set correctly per phase in base_execution_time_predictor.py.
         replica_config  per-phase  TP/PP/WS/EP
         base_execution_time_predictor.py  phase 
        ...
        # [PD-Aware]  base_execution_time_predictor.py  phase 
    ...
    def _generate_aicb_csv(self):
        """Generate AICB CSV file / AICB CSV"""
        ...
        # Check if AICB directory exists /  AICB 
        ...
        # Check results/workload directory /  results/workload 
    ...
    def _generate_or_find_bs1_csv(
        ...
        other models.
        : DeepSeek-V3-671B prefill  H20 (SM90)  tp>=4  AICB
        profiling ， FlashMLA flash_mla_sparse_fwd kernel  h_q
         (B_H=64)。 decode 。
        
        Strategy / :
          1. If requested bs=1, generate directly /  bs=1, 
          2. If bs>1, look for existing bs=1 CSV /  bs>1,  bs=1 CSV
          3. If bs=1 CSV missing, generate it /  bs=1 CSV , 
        
        Args:
            model_name: Model name / 
            ws, tp, pp, ep: Parallelism config / 
            bs: Original requested batch size (may be > 1)
            seq: Sequence length / 
            phase: Stage (prefill/decode) / 
            original_csv_path: Original CSV path (may be bs>1)
        
        Returns:
            Path to found or generated CSV (may be bs=1 CSV)
        """
        logger.debug(f'[AICB Safe Mode] CSV: {original_csv_path}')
        logger.debug(f'[AICB Safe Mode] : model={model_name}, bs={bs}, seq={seq}, phase={phase}')
        
        # ---- Case 1: bs=1 already, generate directly ----
        # ---- 1:  bs=1,  ----
        if bs == 1:
            logger.debug(f'[AICB Safe Mode] bs=1, ...')
            ...
            if self._generate_aicb_csv():
                if os.path.exists(original_csv_path):
                    ...
                    logger.warning('[AICB Safe Mode]  CSV ()')
            else:
                logger.warning('[AICB Safe Mode] bs=1 ')
            return original_csv_path
        
        # ---- Case 2: bs > 1, skip original bs, use bs=1 instead ----
        # ---- 2: bs > 1,  bs,  bs=1 ----
        logger.debug(f'[AICB Safe Mode] bs={bs} > 1, bs (CUDA kernel),  bs=1')
        ...
        self._replica_config.batch_size = original_bs  # Restore immediately / 
        
        logger.debug(f'[AICB Safe Mode] bs=1 CSV : {bs1_csv_path}')
        
        # Check if bs=1 CSV already exists
        #  bs=1 CSV 
        if os.path.exists(bs1_csv_path):
            logger.debug('[AICB Safe Mode]  bs=1 CSV ()')
            return bs1_csv_path
        ...
        self._replica_config.batch_size = original_bs  # Restore / 
        
        if gen_ok and os.path.exists(bs1_csv_path):
            logger.debug(f'[AICB Safe Mode] Successfully generated bs=1 CSV: {bs1_csv_path}')
            return bs1_csv_path
        else:
            logger.warning('[AICB Safe Mode] bs=1  CSV')
            return original_csv_path  # ,  fallback 
    ...
    def _load_aicb_data(self) -> Dict[int, Dict[str, Dict[str, float]]]:
        ...
        # === 3: ，/CSV ===
        logger.debug(f"[AICB] Cache miss, loading CSV (，CSV): "
        ...
        if not os.path.exists(full_csv_path):
            ...
            if not os.path.exists(full_csv_path):
                ...
                # [AICB Fallback]  results/workload/ CSV
                #
    ...
    def _ensure_decode_endpoint_preloaded(self, phase: str, current_seq: int):
        ...
        Args:
            phase: Current stage ("prefill" or "decode") / 
            current_seq: Current iteration's seq value / seq
        """
    ...
    def _preload_decode_endpoint(self, last_seq: int):
        ...
        Args:
            last_seq: Seq_len value of the last decode round / decodeseq_len
        """
        ...
        if os.path.exists(csv_path):
            ...
            if data:
                ...
                # Copy CSV to cache directory / CSV
                try:
                    import shutil
                ...
                logger.warning("[AICB] last_seq CSV")
                _FAILED_PRELOAD_KEYS.add(exact_key)
        else:
            logger.warning(f"[AICB] last_seq CSV: {csv_path}")
            _FAILED_PRELOAD_KEYS.add(exact_key)
        
        # Restore original seq_len / seq_len
    ...
    def _parse_aicb_csv(self, csv_path: str) -> Dict[int, Dict[str, Dict[str, float]]]:
        ...
        try:
            with open(csv_path, newline='') as f:
                ...
                logger.debug(f"[AICB] CSV: {reader.fieldnames}")
            ...
            logger.error(f"CSV: {e}", exc_info=True)
            return {}
