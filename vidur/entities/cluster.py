...
class Cluster(BaseEntity):
    ...
    def __init__(
        ...
        - pd_node_ratio == 1: MIXED mode, all replicas handle both prefill & decode
          pd_node_ratio == 1: MIXED ， replica  prefill  decode
        - 0 < pd_node_ratio < 1: PD separation, independent prefill/decode clusters
          0 < pd_node_ratio < 1: PD ， prefill/decode 
        """
        ...
        if rc.pd_node_ratio == 1:
            ...
            # PD replica  prefill/decode,  world_size
            rc.prefill_world_size = full_ws
            rc.decode_world_size  = full_ws
            rc.prefill_ep = full_ws
            rc.decode_ep  = full_ws
            rc._num_prefill_replicas = num_replicas  # All replicas do prefill /  replica  prefill
            rc._num_decode_replicas  = num_replicas  # All replicas do decode /  replica  decode
        ...
        # Prefill/Decode ,  TP/PP/EP
        #
        # Replica count priority / replica :
        ...
        # ============================================================
            ...
            if rc.num_prefill_replicas is not None:
                ...
                replica_source = f"num_prefill_replicas={rc.num_prefill_replicas} (user specified, )"
            else:
                # Calculate from pd_node_ratio /  pd_node_ratio 
                num_p = int(num_replicas * rc.pd_node_ratio)
                num_d = num_replicas - num_p
                replica_source = f"pd_node_ratio={rc.pd_node_ratio} (by ratio, )"
