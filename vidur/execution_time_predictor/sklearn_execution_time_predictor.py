...
class SklearnExecutionTimePredictor(BaseExecutionTimePredictor):
    ...
    def _predict_for_compute_models_by_aicb(self) -> Dict[str, Any]:
        ...
        model_names = [
            ...
            "mlp_down_proj",
            "mlp_act",
            # "attn_rope",  # RoPE layer modeling not enabled / RoPE
            "attn_kv_cache_save",
            "input_layernorm",
            "post_attention_layernorm",
            "add",
        ...
        # Construct input DataFrame / DataFrame
        ...
        for model_name in model_names:
            ...
            if model_name in ["attn_pre_proj", "attn_post_proj"]:
                ...
                # LayerNorm：0.0003ms，token0.00003ms
                predictions[model_name] = {
                ...
                # All-reduce：0.02ms，token0.002ms
                predictions[model_name] = {
    ...
    def _predict_for_compute_models(self) -> Dict[str, Any]:
        ...
        # [1, max_tokens]batched_token_sizemodel，
    ...
    def _predict_for_cpu_overhead_models_by_aicb(self) -> Dict[str, Any]:
        # Skip if CPU overhead modeling is disabled / CPU，
        if self._config.skip_cpu_overhead_modeling:
            return {}

        # Store CPU overhead predictions / CPU
        predictions = {}

        # CPU-related overhead model names / CPU
        model_names = [
            "schedule",
            "sampler_e2e",
            "prepare_inputs_e2e",
        ...
        # Predict for each CPU overhead model / CPU
        # for model_name in model_names:
        #     model = self._models[model_name]
        #     predictions[model_name] = self._get_model_prediction(model_name, model, X)
        ...
        # Predict for each CPU overhead model / CPU
        for model_name in model_names:
            # Generate simulated values for each model / 
            if model_name == "schedule":
                ...
                # Ray：0.2ms，batch0.02ms
                predictions[model_name] = {
                    (batch_size,): 0.2 + 0.02 * batch_size
                    for batch_size in batch_size_range
                }

        # Return CPU overhead predictions / CPU
        return predictions
    ...
    def _predict_for_attention_layer_models_by_aicb(self) -> Dict[str, Any]:
        # Store attention layer predictions / 
        predictions = {}

        # Decode batch size enumeration range / batch size
        ...
        decode_kv_cache_size_range = np.arange(
            ...
            self._config.kv_cache_prediction_granularity,       # KV cache stored in blocks / kv cacheblock，block_size
        )
        # Decode prefill_chunk_size is fixed at 0 / prefill chunk size0
        ...
        # Prefill chunk size from 1 to max / Prefill chunk1
        ...
        # Split into prefill and decode subsets / prefilldecode
        prefill_df = attention_df[~attention_df["is_decode"]]
        decode_df = attention_df[attention_df["is_decode"]]
        # Filter prefill data with existing cache / prefill
        chunked_prefill_df = prefill_df[prefill_df["kv_cache_size"] > 0].copy()
        # Calculate total prefill token count / prefill token
        chunked_prefill_df["total_prefill_tokens"] = (
            chunked_prefill_df["kv_cache_size"]
            + chunked_prefill_df["prefill_chunk_size"]
        )
        ...
        for kv_cache_size, prefill_chunk_size_squared in prefill_data:
            ...
            # Prefillkv_cachechunk
            # 0.01ms + kv_cache1000.001ms + chunk_size_squared100000.0001ms
        ...
        for batch_size, kv_cache_size in decode_data:
            ...
            # Decodebatch_sizekv_cache
            # 0.005ms + batch_size10.002ms + kv_cache_size1000.0005ms
        ...
        # Return all attention layer predictions / 
        return predictions
    ...
    def _predict_for_attention_layer_models(self) -> Dict[str, Any]:
        ...
        # kv_cache_sizebatch_sizedecode attn
        # kv_cache_sizedecode + prefill，
    ...
    # profilingallreducebytes
    # (num_token -> allreduce time) 
