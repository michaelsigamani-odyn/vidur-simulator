...
class BaseExecutionTimePredictor(ABC):
    ...
    def get_execution_time(self, batch: Batch, pipeline_stage: int) -> ExecutionTime:
        ...
        if self._config.backend == "aicb":
            ...
            # [AICB Backend]  per-batch  replica_config 
            ...
            #
            # [PD-Aware]  phase  TP/PP/WS/EP
            # PD : prefill/decode  world_size  EP
            #  PD : ws = tp * pp * total_dp, ep = ws
            # ============================================================
            ...
            if replica_config.phase == "prefill" and hasattr(self._replica_config, 'prefill_world_size'):
                # PD separation: use prefill cluster params / PD :  prefill 
                ...
                # PD separation: use decode cluster params / PD :  decode 
            ...
            #  batch  AICB , 
            # : PD prefill_world_size (),  pd_node_ratio 
            ...
            if replica_config.phase == "prefill":
                ...
                bs = 0
                seq = 0
                decode_last_seq = 0  # Last decode iteration's seq for first-last interpolation / decodeseq，
                for request, num_tokens_to_process in zip(batch.requests, batch.num_tokens):
                    if request._is_prefill_complete:
                        bs += 1
                        # Current iteration seq = prefill_tokens + processed_decode_tokens - 1
                        # seq = prefill_tokens + processed_decode_tokens - 1
                        ...
                        # seq = prefill_tokens + (decode_tokens - 1) - 1
                        #  processed_decode_tokens = decode_tokens - 1
